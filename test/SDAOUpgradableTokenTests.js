...
var  SDAORewardToken = artifacts.require("./SDAORewardToken.sol");
...
contract('SDAOUpgradableToken', function(accounts) {

    var sdaoRewardToken;
    
    before(async () => 
        {
            sdaoRewardToken = await SDAORewardToken.deployed();
        });

        const getInitialSupplyAndVerify = async (_totalSupply) => {
            
            const totalSupply = await sdaoRewardToken.totalSupply.call()
        ...
        const getDecimalsAndVerify = async (_decimals) => {
            ...
            const decimals = await sdaoRewardToken.decimals.call()
        ...
        const mintAndVerify = async (_account, _amount) => {
            ...
            const totalSupply_b = await sdaoRewardToken.totalSupply.call()
            const wallet_bal_b = (await sdaoRewardToken.balanceOf(_account));

            const _amountBN = new BigNumber(_amount);

            await sdaoRewardToken.mint(_account, _amountBN, {from:_account})

            const totalSupply_a = await sdaoRewardToken.totalSupply.call();
            const wallet_bal_a = (await sdaoRewardToken.balanceOf(_account));
        ...
        const transferAndVerify = async (_accountFrom, _accountTo, _amount) => {
            ...
            const sender_bal_b = (await sdaoRewardToken.balanceOf(_accountFrom));
            const receiver_bal_b = (await sdaoRewardToken.balanceOf(_accountTo));

            await sdaoRewardToken.transfer(_accountTo, _amountBN.toString(), {from:_accountFrom})

            const sender_bal_a = (await sdaoRewardToken.balanceOf(_accountFrom));
            const receiver_bal_a = (await sdaoRewardToken.balanceOf(_accountTo));
        ...
        const pauseContractAndVerify = async (_accountFrom) => {
            ...
            await sdaoRewardToken.pause({from:_accountFrom});
            const paused = (await sdaoRewardToken.paused());
        ...
        const unPauseContractAndVerify = async (_accountFrom) => {
            ...
            await sdaoRewardToken.unpause({from:_accountFrom});
            const paused = (await sdaoRewardToken.paused());
        ...
        const burnAndVerify = async (_amount, _accountFrom) => {
            ...
            await sdaoRewardToken.burn(_amountBN.toString(), {from:_accountFrom});
        ...
        const getPauserRole = async () => {
            ...
            return await sdaoRewardToken.PAUSER_ROLE.call();
        }

        const grantPauseRole = async (_accountFrom, _pauserAccount) => {

            const pauseRole = await getPauserRole();
            await sdaoRewardToken.grantRole(pauseRole, _pauserAccount, {from:_accountFrom});

        }

        const grantMinterRole = async (_accountFrom, _minterAccount) => {

            const minterRole = await sdaoRewardToken.MINTER_ROLE.call();
            await sdaoRewardToken.grantRole(minterRole, _minterAccount, {from:_accountFrom});
    ...
    it("1. Mint Token - First & subsequent mints", async function() 
    {
        ...
        await testErrorRevert(sdaoRewardToken.mint(accounts[1], mintAmountBN, {from:accounts[1]}));        
    ...
    it("3. Admin Functionality - Pause and Resume validation", async function() 
    {
        ...
        await testErrorRevert(sdaoRewardToken.pause({from:accounts[1]}));
    ...
    it("5. Admin Functionality - Assign Minter Role and Validate", async function() 
    {
        ...
        await testErrorRevert(sdaoRewardToken.mint(accounts[0], mintAdditionalAmtBN, {from:accounts[8]}));
